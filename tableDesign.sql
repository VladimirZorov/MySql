CREATE DATABASE `restaurant_db_Database`;

USE `restaurant_db_Database`;
...
`order_id` INT,
`client_id` INT,
CONSTRAINT `fk_oc_orderes` FOREIGN KEY (`order_id`)
...
`order_id` INT,
`product_id` INT,
CONSTRAINT `fk_op_products1` FOREIGN KEY (`order_id`)
    REFERENCES `orders` (`id`),
